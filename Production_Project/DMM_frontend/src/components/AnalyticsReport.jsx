...
const HIGHLIGHT_PRIORITY = ['followers', 'interactions', 'subscribers', 'impressions', 'engagementRate', 'newFollowers', 'reach', 'views', 'pageViews'];
const fmt = (v, isPct) => (isPct ? `${Number(v || 0).toFixed(1)}%` : formatNumber(v || 0));
const fmtDate = (d) => new Date(d).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });

// Platform-aware guidance for the empty state — read-only wording, since data
// entry (Meta/YouTube sync, Excel imports, manual entry) lives in the admin portal.
const EMPTY_HINTS = {
  Instagram: 'Data appears once your administrator syncs this page from Meta or imports metrics.',
  Facebook: 'Data appears once your administrator syncs this page from Meta or imports metrics.',
  YouTube: 'Data appears once your administrator syncs this channel from YouTube or imports metrics.',
  LinkedIn: 'Data appears once your administrator imports a LinkedIn export or enters metrics.',
};
...
export default function AnalyticsReport({ report, isLoading }) {
  ...
  if (!report?.hasData) {
    return (
      <EmptyState icon={TrendingUp} title="No analytics yet"
        description={EMPTY_HINTS[report?.platform] || "Your administrator hasn't entered metrics for this platform yet. Once they do, you'll see week-over-week performance here."} />
    );
  }
  ...
  const tipFor = (f) => {
    const suffix = !sync.provider ? ''
      ...
      : ` Not filled by ${sync.provider} auto-sync — it arrives via the weekly Excel import or manual entry.`;

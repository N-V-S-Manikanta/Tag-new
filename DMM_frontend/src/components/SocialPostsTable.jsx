...
// Metric columns per platform — Instagram is richest; Facebook has no reliable
...
const PLATFORM_COLOR = { Instagram: '#E1306C', Facebook: '#1877F2', YouTube: '#FF0000' };

export default function SocialPostsTable({ orgId, platform }) {
  const qc = useQueryClient();
  const { user } = useAuthStore();
  const canSync = ['ADMIN', 'CEO'].includes(user?.role);
  const color = PLATFORM_COLOR[platform] || '#f15d27';
              ...
              {posts.map((p) => (
                <tr key={p._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/40">
                  <td className="px-5 py-2.5">
                    <a href={p.url || '#'} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-left group">

...
export function DeltaBadge({ delta, isPct }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-semibold', cls)}>
...
export default function AnalyticsReport({ report, isLoading }) {
  ...
  if (!report?.hasData) {
    return <EmptyState icon={TrendingUp} title="No analytics yet" description="Your administrator hasn't entered metrics for this platform yet. Once they do, you'll see week-over-week performance here." />;
  }
  ...
  return (
    <div className="space-y-5">
      {previous && (
        <p className="text-xs text-slate-400">Comparing latest entry ({fmtDate(latest.date)}) with previous ({fmtDate(previous.date)})</p>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={series} margin={{ left: -12, right: 8, top: 8 }}>
              ...
              <Area type="monotone" dataKey={audienceField} stroke="#6366f1" strokeWidth={2.5} fill="url(#aud)" name={labels[audienceField]} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={series} margin={{ left: -12, right: 8, top: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
              <XAxis dataKey="date" tickFormatter={fmtDate} tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} minTickGap={20} />
              <YAxis tick={{ fontSize: 12, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => formatNumber(v)} />
